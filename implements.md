...
- 구현 중 `gameStore.ts` 테스트가 필요할 정도로 로직이 생기면 기존 테스트 파일 하나에 저장 검증을 추가하고, 파일 분리는 실제 크기 증가 시에만 한다.
...
8. 별도 플레이에서 패배 후 보상이 없고 준비 화면으로 복귀하는지 확인한다.
9. 새로고침 후 프로필과 누적 보상만 복구되고 진행 중 세션은 복구되지 않는지 확인한다.
10. iOS Safari와 Android Chrome의 가로 화면에서 터치, safe area, 주소창 높이 변화, 스크롤 억제를 확인한다.
...
- iOS Safari 및 Android Chrome 실기기 플레이 루프 확인.
...
- 처음부터 승리/패배 결과와 준비 복귀까지 진행 가능.
- 탐사·조우·리롤·피해·보상·저장이 실제 단일 상태에 반영됨.
- 모바일 가로 화면의 두 우선 브라우저에서 진행 차단 오류가 없음.
