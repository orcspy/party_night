...
- iOS Safari 지원과 로컬 HTTPS·인증서 구성. 현 MVP 우선 지원 대상은 Android Chrome으로 한정한다.
...
10. Android Chrome 실기기의 가로 화면에서 터치, safe area, 주소창 높이 변화, 스크롤·확대·텍스트 선택 억제를 확인한다.
...
- Android Chrome 실기기 플레이 루프 확인. 태블릿 검증 후 가능하면 소형 스마트폰 가로 화면을 추가 확인한다.
- iOS Safari는 현 단계 완료 기준과 수정 범위에서 제외한다.
...
- Android Chrome 가로 화면에서 진행 차단 오류가 없음.
