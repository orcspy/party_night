...
export function ResultScreen({ state, dispatch }: { state: GameState; dispatch: (command: GameCommand) => void }) {
  ...
  return (
    <main className={`menu-screen result-screen ${victory ? 'victory' : 'defeat'}`}>
      <div className="result-scroll-region" role="region" aria-label="원정 결과 상세" tabIndex={0}>
        <div className="result-content">
          ...
          <div className="reward-card">
            <span>획득 골드<strong>+{state.result?.gold ?? 0}</strong></span>
            <span>획득 경험치<strong>+{state.result?.experience ?? 0}</strong></span>
          </div>
          ...
          {victory && rewardEntries.length > 0 && !pending && (
            <section className="settlement-details">
              <h2>원정 보상</h2>
              <div className="content-inline-list reward-list">
          ...
          {pending && (
            <section className="reward-selection">
              <h2>창고 초과 보상 선택</h2>
              <p>창고 {usedStorageSlots(state.profile!)}/100 · 보관할 보상을 선택하세요.</p>
              {pending.rewards.map((reward) => {
                const presentation = getRewardPresentation(reward)
                return (
                  <label key={reward.rewardId} data-rarity={presentation.rarity}>
                    <input type="checkbox" checked={selectedIds.has(reward.rewardId)} onChange={() => {
                    ...
                    <span className="content-identity"><ContentIcon {...presentation} size="small" /><span className="content-name">{presentation.label}</span>{presentation.rarityLabel && <span className="rarity-badge">{presentation.rarityLabel}</span>}</span>
                  </label>
                )
              })}
            </section>
      ...
      <footer className="result-actions" aria-label="결과 화면 동작">
        {pending ? (
          <button className="danger" onClick={() => dispatch({ type: 'CONFIRM_REWARD_SELECTION', confirmDiscardUnselected: true })}>선택 보관·나머지 포기</button>
        ) : (
