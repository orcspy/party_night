...
export function ResultScreen({ state, dispatch }: { state: GameState; dispatch: (command: GameCommand) => void }) {
  ...
  return (
    <main className={`menu-screen result-screen ${victory ? 'victory' : 'defeat'}`}>
      ...
      {summary && <section className="settlement-details"><h2>성장·해금</h2>{summary.characterResults.map((result) => <p key={result.characterId}>{state.profile?.characters.find((character) => character.characterId === result.characterId)?.name}: Lv {result.previousLevel} → {result.level} · EXP +{summary.experiencePerCharacter} · 신규 스킬 {result.unlockedClassSkillIds.map(getSkillDisplayName).join(', ') || '없음'}</p>)}<p>{summary.unlockedQuestIds.length > 0 ? `${summary.unlockedQuestIds.map(getQuestDisplayName).join(' · ')} 해금` : '추가 퀘스트 해금 없음'}</p>{summary.unlockedRarities.length > 0 && <p>신규 장비 등급: {summary.unlockedRarities.map((rarity) => rarity === 'heroic' ? '영웅' : rarity === 'rare' ? '희귀' : rarity === 'uncommon' ? '고급' : rarity).join(' · ')}</p>}{summary.questId === 'underground_dungeon_quest' && <p>만능 치료제가 상점에 추가되었습니다.</p>}</section>}
