  ...
  return (
    <div className="management-panel character-management">
      ...
      <div className="management-list compact">{character.customSkillSlots.map((slot, index) => {
        ...
        return <article key={index}><span className="content-copy"><b>커스텀 {index + 1}</b>{slot && presentation ? <span className="content-identity"><ContentIcon {...presentation} size="small" /><small>{getSkillDisplayName(slot.skillId)}</small></span> : <small>{character.level >= [3,7,10][index] ? '비어 있음' : `Lv${[3,7,10][index]} 잠금`}</small>}</span>{slot && <button onClick={() => dispatch({ type: 'UNEQUIP_CUSTOM_SKILL', characterId: character.characterId, skillInstanceId: slot.skillInstanceId, slotIndex: index })}>해제</button>}</article>
      })}</div>
      <h3>장착 장비</h3>
      <div className="management-list compact">
        {SLOTS.map((slot) => {
          ...
          return <article key={slot} className={presentation ? 'content-row' : undefined} data-rarity={presentation?.rarity}><span className="content-copy"><b>{getEquipmentSlotDisplayName(slot)}</b>{instance && presentation ? <span className="content-identity"><ContentIcon {...presentation} size="small" /><small><span className="content-name">{getEquipmentDisplayName(instance.equipmentId)}</span> <span className="rarity-badge">{presentation.rarityLabel}</span></small></span> : <small>비어 있음</small>}</span>{instance && <button onClick={() => dispatch({ type: 'UNEQUIP_ITEM', characterId: character.characterId, slot, equipmentInstanceId: instance.equipmentInstanceId })}>해제</button>}</article>
        })}
      </div>
